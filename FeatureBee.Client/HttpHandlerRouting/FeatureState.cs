namespace FeatureBee.HttpHandlerRouting
{
    ...
    public class FeatureState : IHandleARoute
    {
        ...
        public void DoHandleRoute(HttpContext context)
        {
            ...
            {
                context.Response.Write(serializer.Serialize(new { success = false, name = featureName, message = "the query parameter for name is missing" }));
            }
            else
            {
                var isEnabled = Feature.IsEnabled(featureName);
                context.Response.Write(serializer.Serialize(new { success = true, name = featureName, isEnabled }));
            }
